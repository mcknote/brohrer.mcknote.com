...
最後，CNNs 還有一項秘密武器——**全連結層**（fully connected layers）會集合高層中篩選後的圖片，並把圖片所包含的資訊轉化為票數。在我們的例子裡有兩個選項：圈和叉。全連結層就是傳統神經網路架構中的**主要建構單元**（primary building block），它會將矩陣中的所有值當成一個一維清單，而不是當成一個二維矩陣。這個清單裡的每個值都可以決定圖片中的符號是圈還是叉，

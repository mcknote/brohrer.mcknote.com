...
另一個 CNNs 所使用的強大工具是**池化**（pooling）。池化是一個將原本的大圖壓縮並保留重要資訊的方法，它的運作原理只需要小二的數學程度就能弄懂。池化會在圖片上的各個**位置**（step）選取不同**範圍**（window），並在這個範圍中選擇一個最大值。實務上，邊長為二或三的正方形範圍，搭配兩像素的**移動距離**（stride）是滿理想的設定。